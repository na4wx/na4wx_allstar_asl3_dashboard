...
// applyDirectReplyRedirect exists because inbound delivery has to stay
// on kernel-level DNAT (see addNat in the cloud repo's relayNetwork.ts)
// rather than a userspace relay: app_rpt's own parse_caller check
// (app_rpt.c) rejects a call whose channel peer address doesn't match
// the calling node's own real, directory-registered IP -- confirmed on
// a real deployment ("Node N IP a.b.c.d does not match link IP
// e.f.g.h!!"), when a userspace relay was tried for *both* directions
// and chan_iax2's peer became the cloud's own rendezvous address
// instead of the real caller. DNAT preserves the caller's real source
// address, satisfying that check, but that in turn means chan_iax2's
// own reply is naturally addressed back to that same real caller --
// and sending that reply, unmodified, out this node's own tunnel
// interface was proven, via a controlled, isolating test on a real
// deployment, to be lost between this node and the cloud, every single
// time, while a genuinely new, outbound-initiated call (no pre-existing
...
// sustained two-way traffic. The root cause was never conclusively
// identified despite ruling out NAT clash resolution, AllowedIPs,
// interface-level drops on both ends, the FORWARD chain, routing,
// kernel UDP-layer errors, WireGuard handshake stability, and conntrack
// state -- though two later, unrelated discoveries on the cloud side
// (a stale-rule accumulation bug and a missing INPUT-chain allow, both
// now fixed) may well have been the real explanation for at least some
// of that -- worth retesting the in-tunnel reply again if this
// mechanism itself ever needs revisiting.
//
